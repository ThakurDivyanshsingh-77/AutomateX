...
export class DiscordApiClient {
  ...
  constructor(config: IDiscordApiClientConfig) {
    if (!config.botToken) {
  ...
  public async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    while (attempt <= this.maxRetries) {
      ...
      try {
        ...
        // Handle Errors
        let responseBody: IDiscordApiErrorResponse | undefined;
        try {
          responseBody = (await response.json()) as IDiscordApiErrorResponse;
        } catch {
        ...
        // Throw normalized error for non-retriable statuses or exceeded retries
        ...
        // If it's already a normalized error, rethrow
        if (typeof err === 'object' && err !== null && 'statusCode' in err) {
          throw err;
        }

        // Network abort/timeout or connection failure
        if (attempt <= this.maxRetries) {
          const backoffMs = Math.pow(2, attempt) * 500;
          console.warn(`[DiscordApiClient] Network/Fetch error on ${endpoint}: ${(err as Error).message}. Retrying in ${backoffMs}ms`);
          await DiscordUtils.sleep(backoffMs);
          continue;
        }
