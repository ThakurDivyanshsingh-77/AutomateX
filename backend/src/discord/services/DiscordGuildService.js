...
export class DiscordGuildService {
  ...
  /**
   * Force refresh guilds by invalidating cache.
   */
  static async refreshGuilds(ownerId, credentialId) {
   ...
   */
  static async validateGuild(ownerId, credentialId, guildId) {
    try {
      const res = await this.getGuilds(ownerId, credentialId, false);
      return res.guilds.some((g) => g.id === guildId || g.value === guildId);
    } catch (err) {
      console.warn(`[DiscordGuild] ❌ Guild validation failed for Guild ID "${guildId}": ${err.message}`);
      return false;
    }
  }
   ...
   */
  static clearCache(ownerId, credentialId) {
