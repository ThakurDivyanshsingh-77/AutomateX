import { IDiscordEmbed, IDiscordEmbedField, IDiscordSendEmbedInput } from '../types/DiscordTypes.js';

export interface IValidationResult {
  isValid: boolean;
  errors: string[];
  formattedEmbed?: IDiscordEmbed;
}

export class DiscordEmbedValidators {
  /**
   * Convert hex color string (e.g. "#5865F2", "5865F2", "#00FF00") or number to decimal integer for Discord API.
   */
  public static parseColor(color: string | number | undefined | null): number | undefined {
  ...
  /**
   * Validate full embed payload against Discord REST API limits.
   */
  public static validateEmbedInput(input: IDiscordSendEmbedInput): IValidationResult {
    const errors: string[] = [];
    ...
    // At least one content element required
    ...
    // Color Validation
    let parsedColor: number | undefined;
    ...
    const formattedFields: IDiscordEmbedField[] = [];
    ...
    // Assemble clean formatted embed object for Discord REST API
    const formattedEmbed: IDiscordEmbed = {};
