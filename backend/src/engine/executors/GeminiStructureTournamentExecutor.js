...
- Return valid JSON only without markdown formatting.`;
  ...
  static parseNumber(val, defaultVal = null) {
    ...
    if (typeof val === 'string') {
      ...
      // Strip currency symbols (₹, $, €, £, ¥, etc.), commas, spaces, and text like (INR), (Rs), (₹)
  ...
  static normalizeTournamentList(rawList) {
    const normalizedTournaments = rawList.map((t) => {
      const firstPrize = this.parseNumber(
        t.firstPrize || t.first || t.prizeBreakdown?.first || t['1stPlacePrize'] || t['1st Place Prize'] || t['1st Prize'] || t['1st'],
        null
      );
      const secondPrize = this.parseNumber(
        t.secondPrize || t.second || t.prizeBreakdown?.second || t['2ndPlacePrize'] || t['2nd Place Prize'] || t['2nd Prize'] || t['2nd'],
        null
      );
      const thirdPrize = this.parseNumber(
        t.thirdPrize || t.third || t.prizeBreakdown?.third || t['3rdPlacePrize'] || t['3rd Place Prize'] || t['3rd Prize'] || t['3rd'],
        null
      );

      let prizePool = this.parseNumber(
        t.prizePool ?? t['Total Prize Pool'] ?? t['Total Prize Pool (₹)'] ?? t.totalPrizePool ?? t.totalPrize ?? t['Total Prize'] ?? t['Prize Pool'] ?? t.prize_pool,
        null
      );

      if (prizePool === null && (firstPrize !== null || secondPrize !== null || thirdPrize !== null)) {
        prizePool = (firstPrize || 0) + (secondPrize || 0) + (thirdPrize || 0);
      }

      let winnerCount = this.parseNumber(
        t.winnerCount ?? t['Winner Count'] ?? t.winners ?? t['Total Winners'] ?? t.winner_count,
        null
      );
      if (winnerCount === null) {
        if (thirdPrize !== null && thirdPrize > 0) winnerCount = 3;
        else if (secondPrize !== null && secondPrize > 0) winnerCount = 2;
        else if (firstPrize !== null && firstPrize > 0) winnerCount = 1;
        else winnerCount = 3;
      }

      const prizeBreakdown = {
        first: firstPrize !== null ? firstPrize : (prizePool ? Math.round(prizePool * 0.5) : 0),
        second: secondPrize !== null ? secondPrize : (prizePool ? Math.round(prizePool * 0.3) : 0),
        third: thirdPrize !== null ? thirdPrize : (prizePool ? Math.round(prizePool * 0.2) : 0),
      };

      const entryFee = this.parseNumber(
        t.entryFee ?? t['Entry Fee'] ?? t['Entry Fee (₹)'] ?? t.fee ?? t.entry_fee,
        0
      );

      const slots = this.parseNumber(
        t.slots ?? t['Max Capacity Slots'] ?? t['Max Capacity'] ?? t.maxCapacity ?? t.maxTeams ?? t.maxSlots ?? t.capacity ?? t.totalSlots ?? t['Total Slots'],
        null
      );

      return {
        title: this.cleanString(t.title || t.name || t.tournamentName || t['Tournament Title'] || t['Tournament Name'], 'title'),
        game: this.cleanString(t.game || t.gameName || t['Game Name'] || t['Game'], 'game'),
        mode: this.cleanString(t.mode || t.gameMode || t.tournamentMode || t['Mode'] || t['Game Mode'], 'mode'),
        entryFee,
        prizePool,
        winnerCount,
        firstPrize: firstPrize !== null ? firstPrize : prizeBreakdown.first,
        secondPrize: secondPrize !== null ? secondPrize : prizeBreakdown.second,
        thirdPrize: thirdPrize !== null ? thirdPrize : prizeBreakdown.third,
        prizeBreakdown,
        slots,
        date: this.cleanString(t.date || t.tournamentDate || t.eventDate || t['Tournament Date'] || t['Date'], 'date'),
        time: this.cleanString(t.time || t.startTime || t.eventTime || t['Start Time'] || t['Time'], 'time'),
        map: this.cleanString(t.map || t.mapName || t.playingMap || t['Map Name'] || t['Map'], 'map'),
        bannerImage: this.cleanString(t.bannerImage || t.bannerImageUrl || t.bannerUrl || t.imageUrl || t['Banner Image URL'] || t['Banner Image'], 'bannerImage'),
        description: this.cleanString(t.description || t.rules || t.details || t['Description & Rules'] || t['Description'], 'description'),
      };
    });
  ...
  async execute(node, context) {
    ...
    tournaments.forEach((t, i) => {
      console.log(`   ${i + 1}. ${t.title || 'Untitled'} (Game: ${t.game || 'N/A'}, Mode: ${t.mode || 'N/A'}, PrizePool: ${t.prizePool ?? 'N/A'}, EntryFee: ${t.entryFee ?? 0}, Map: ${t.map || 'N/A'}, Slots: ${t.slots || 'N/A'})`);
    });
  ...
  deterministicExtract(text) {
    ...
    const title = getValue([
      /(?:Tournament\s*(?:Title|Name)?|Title)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r]+)/i,
    ...
    const mode = getValue([
      /(?:Mode|Game\s*Mode|Tournament\s*Mode)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r,]+)/i,
    ]);

    const prizePoolStr = getValue([
      /(?:Total\s*Prize\s*Pool|Total\s*Prize|Prize\s*Pool)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r]+)/i,
    ]);

    const entryFeeStr = getValue([
      /(?:Entry\s*Fee|Fee)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r]+)/i,
    ]);

    const slotsStr = getValue([
      /(?:Max\s*Capacity\s*Slots|Max\s*Capacity|Capacity|Total\s*Slots|Slots|Max\s*Teams|Teams)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*(\d+)/i,
    ]);

    const winnerCountStr = getValue([
      /(?:Winner\s*Count|Winners|Total\s*Winners)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*(\d+)/i,
    ]);

    const firstPrizeStr = getValue([
      /(?:1st\s*Place\s*Prize|1st\s*Prize|First\s*Place\s*Prize|First\s*Prize|1st)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r]+)/i,
    ]);

    const secondPrizeStr = getValue([
      /(?:2nd\s*Place\s*Prize|2nd\s*Prize|Second\s*Place\s*Prize|Second\s*Prize|2nd)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r]+)/i,
    ]);

    const thirdPrizeStr = getValue([
      /(?:3rd\s*Place\s*Prize|3rd\s*Prize|Third\s*Place\s*Prize|Third\s*Prize|3rd)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r]+)/i,
    ]);

    const date = getValue([
      /(?:Tournament\s*Date|Event\s*Date|Date)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*(\d{4}-\d{2}-\d{2}|\d{2}\/\d{2}\/\d{4})/i,
    ]);

    const time = getValue([
      /(?:Start\s*Time|Event\s*Time|Time)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*(\d{1,2}:\d{2}(?:\s*[AP]M)?)/i,
    ]);

    const map = getValue([
      /(?:Map\s*Name|Playing\s*Map|Map)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r,]+)/i,
    ]);

    const bannerImage = getValue([
      /(?:Banner\s*Image\s*URL|Banner\s*(?:Image|URL)?|Image\s*URL)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*(https?:\/\/[^\s\n\r]+)/i,
    ]);

    const description = getValue([
      /(?:Description\s*(?:&|\+)?\s*Rules|Description|Rules|Details|Overview)\s*(?:\([^)]*\))?\s*[:=\t|-]\s*([^\n\r]+(?:\n[^\n\r]+)*)/i,
    ]);
    ...
    const entryFee = GeminiStructureTournamentExecutor.parseNumber(entryFeeStr, 0);
