...
- **First-Class AutomateX Node (`githubSyncProfileReadme`)**:
  ...
  - **Central & Legacy Registry Registration**: Registered in both central runtime `backend/src/engine/registry/ExecutorRegistry.js` (`WorkflowEngine.run`) and legacy `backend/src/engine/ExecutorRegistry.js` (`ExecutionEngine.executeWorkflow`). Supports node aliases: `githubSyncProfileReadme`, `github_sync_profile_readme`, `githubSyncReadme`, `github`.
  - **Non-Destructive Markers**: Strictly replaces only content between markers (`<!-- AUTOMATEX:PROJECTS:START -->` / `<!-- AUTOMATEX_PROJECTS_START -->`), preserving header, bio, and footer. Appends safely if markers are missing without overwriting existing content.
  - **Idempotency & Zero Phantom Commits**: Compares existing file SHA and decoded content; if identical, returns `{ success: true, changed: false, committed: false }` with 0 unnecessary commits.
  - **`GitHubSyncReadmeExecutor.js`**: Real production workflow executor handling resolution of credentials, expressions, loop guards, dryRun execution, and structured error responses.
...
- **Verification**: 11 automated test suites passed (`test_github_sync_profile_readme.js`) including `Cron Schedule -> githubSyncProfileReadme -> End Completion` DAG execution and production frontend build passed cleanly (2,105 modules transformed).
