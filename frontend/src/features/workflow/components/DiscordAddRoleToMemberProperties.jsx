...
import { UserPlus, Loader2, AlertCircle, CheckCircle2, FileText, Shield } from 'lucide-react';

export const DiscordAddRoleToMemberProperties = ({ nodeData, onUpdateConfig }) => {
  ...
  const fetchDiscordCredentials = async () => {
    ...
    try {
      ...
      console.warn('[DiscordAddRoleToMemberProperties] Failed to load credentials:', err);
  ...
  const handleTestAddRole = async () => {
    ...
    try {
      const res = await api.post('/discord/add-role-to-member', {
        credentialId,
        guildId,
        userId,
        roleId,
      ...
      const errMsg = err.response?.data?.message || 'Failed to assign Discord role to member';
      toast.error(errMsg);
    } finally {
      setTesting(false);
    }
    ...
    <div className="space-y-4 text-xs font-sans select-none">
      ...
      <div className="p-3 bg-indigo-950/40 border border-indigo-500/30 rounded-xl flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white text-xs">Discord → Add Role to Member</span>
        </div>
      ...
      <div className="space-y-1.5">
        ...
        {isManualUserInput ? (
          <input
            type="text"
            placeholder="e.g. {{steps['Previous Node'].user.id}} or 1234567890"
            value={userId}
            onChange={(e) => {
              const val = e.target.value;
              setUserId(val);
      ...
      <div className="space-y-1">
        ...
        <input
          type="text"
          placeholder="e.g. Assigned via AutomateX Workflow Engine"
          value={reason}
          onChange={(e) => {
            const val = e.target.value;
            setReason(val);
      ...
      {/* Add Role Test Execution Button */}
      <div className="pt-1">
        <button
          type="button"
          onClick={handleTestAddRole}
        ...
        >
          {testing ? (
            <>
              ...
              <span>Assigning Role on Discord...</span>
            </>
          ) : (
            <>
              <UserPlus className="w-4 h-4" />
              <span>Add Role to Member</span>
            </>
          )}
        </button>
      </div>
      ...
      {testResult && testResult.success && (
        <div className="p-3 bg-emerald-950/40 border border-emerald-500/30 rounded-xl space-y-1.5 text-xs">
          ...
          <div className="text-[11px] font-mono text-slate-300 space-y-0.5">
            ...
            <div>Role ID: <strong className="text-indigo-300">{testResult.roleId}</strong></div>
...
export default DiscordAddRoleToMemberProperties;
