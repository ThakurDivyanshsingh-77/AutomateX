...
import { Trash2, Loader2, AlertTriangle, AlertCircle, CheckCircle2, ShieldAlert } from 'lucide-react';

export const DiscordDeleteChannelProperties = ({ nodeData, onUpdateConfig }) => {
  ...
  const [channelId, setChannelId] = useState(config.channelId || config.channel || '');
  const [confirmDelete, setConfirmDelete] = useState(Boolean(config.confirmDelete));
  const [isManualInput, setIsManualInput] = useState(
    Boolean(channelId && (channelId.includes('{{') || channelId.includes('$')))
  );
  ...
  const fetchDiscordCredentials = async () => {
    ...
    try {
      ...
      console.warn('[DiscordDeleteChannelProperties] Failed to load credentials:', err);
  ...
  const handleTestDeleteChannel = async () => {
    if (!credentialId) return toast.error('Please select a Discord Credential');
    if (!channelId) return toast.error('Please select a Discord Channel or enter a dynamic Channel ID');
    ...
    try {
      const res = await api.post('/discord/delete-channel', {
        credentialId,
        guildId,
        channelId,
        confirmDelete,
      ...
      if (res.data.success) {
        ...
        toast.success(`Channel ${res.data.channel?.name || channelId} Deleted Successfully!`);
      } else {
        toast.error(res.data.message || 'Failed to delete channel');
      }
    } catch (err) {
      const errMsg = err.response?.data?.message || 'Failed to delete Discord channel';
      toast.error(errMsg);
    } finally {
      setTesting(false);
    }
    ...
    <div className="space-y-4 text-xs font-sans select-none">
      ...
      <div className="p-3 bg-rose-950/40 border border-rose-500/30 rounded-xl flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white text-xs">Discord → Delete Channel</span>
        </div>
      ...
      <DiscordServerDropdown
        ...
        onChange={(selectedGuildId) => {
          ...
          if (!isManualInput) setChannelId('');
          updateConfigField('guildId', selectedGuildId);
          if (!isManualInput) updateConfigField('channelId', '');
        }}
      />

      {/* 3. Channel Selector (Dropdown or Variable Expression Input) */}
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <label className="block text-xs font-semibold text-slate-300">
            Discord Channel <span className="text-rose-400">*</span>
          </label>
          <button
            type="button"
            onClick={() => {
        ...
        {isManualInput ? (
          <input
            type="text"
            placeholder="e.g. {{steps['Discord → Create Channel'].channel.id}} or 1234567890"
            value={channelId}
            onChange={(e) => {
              const val = e.target.value;
              setChannelId(val);
      ...
      {/* 4. Permanent Deletion Warning Banner */}
      <div className="p-3 bg-rose-950/60 border border-rose-500/40 rounded-xl space-y-2 text-rose-200">
        ...
        <p className="text-[11px] text-rose-300/90 leading-relaxed">
          Deleting a Discord channel is permanent and cannot be undone. All messages, history, pins, and attachments inside this channel will be permanently removed.
        </p>

        {/* 5. Required Confirmation Checkbox */}
        <label className="flex items-start gap-2 pt-1 text-xs font-semibold text-white cursor-pointer select-none">
          <input
            ...
            className="mt-0.5 rounded border-rose-800 bg-slate-900 text-rose-600 focus:ring-0"
          />
          <span className="leading-tight text-rose-200">
            I understand this channel will be permanently deleted.
          </span>
        </label>
      </div>

      {/* Delete Channel Test Execution Button */}
      <div className="pt-1">
        <button
          type="button"
          onClick={handleTestDeleteChannel}
          disabled={testing || !credentialId || !channelId || !confirmDelete}
        ...
        >
          {testing ? (
            <>
              ...
              <span>Deleting Channel on Discord...</span>
            </>
          ) : (
            <>
      ...
      {testResult && testResult.channel && (
        <div className="p-3 bg-emerald-950/40 border border-emerald-500/30 rounded-xl space-y-1.5 text-xs">
          ...
          <div className="text-[11px] font-mono text-slate-300 space-y-0.5">
            <div>Deleted Channel ID: <strong className="text-white">{testResult.channel.id}</strong></div>
            <div>Channel Name: <strong className="text-slate-400">{testResult.channel.name}</strong></div>
...
export default DiscordDeleteChannelProperties;
